...
public class AccumuloInitRunner extends BaseAccumuloRunner {
  ...
  protected Process initServer() throws Exception {
    ...
    ProcessBuilder processBuilder = initProcessBuilder(null, Arrays.asList("init"));
    buildLibDir(getClasspathEntries(), getBaseDirectory().getCanonicalPath());
  ...
  public AccumuloInitRunner(File baseDirectory, List<String> classpathEntries, String instanceName, String password, int dfsRPCPort, int zooPort) {
  ...
  void buildLibDir(List<String> sourceFiles, String destination) throws IOException {
  ...
  private void writeProperties(File outputFile, Map<String,String> properties) throws XMLStreamException, IOException {
    XMLOutputFactory outputFactory = XMLOutputFactory.newFactory();
    XMLStreamWriter writer = outputFactory.createXMLStreamWriter(new FileOutputStream(outputFile));
  ...
  void createDefaultHadoopConfig(File baseDirectory) throws XMLStreamException, IOException {
    String confPath = baseDirectory.getCanonicalPath() + File.separator + "conf";
    FileUtils.mkdir(confPath);
    File output = new File(confPath, "core-site.xml");
    Map<String,String> props = new HashMap<String,String>();
    props.put("fs.default.name", String.format("hdfs://localhost:%d", dfsRPCPort));
    writeProperties(output, props);
  }
  
  void createDefaultAccumuloConfig(File baseDirectory) throws XMLStreamException, IOException {
    String confPath = baseDirectory.getCanonicalPath() + File.separator + "conf";
    FileUtils.mkdir(confPath);
    String walogPath = baseDirectory.getCanonicalPath() + File.separator + "walog";
    File walogDir = new File(walogPath);
    walogDir.mkdir();
    File output = new File(confPath, "accumulo-site.xml");
    Map<String,String> props = new HashMap<String,String>();
    props.put("instance.zookeeper.host", String.format("localhost:%d", zooPort));
